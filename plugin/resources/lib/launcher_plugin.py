...
class Main:
        ...
        if param:
            ...
            if (len(command_part) == 1):
                ...
                if (category == SEARCH_COMMAND):
                    ...
                    backup_file = xbmcgui.Dialog().browse(1, __language__(30186), "files", ".xml", False, False,
                                                          os.path.join(DEFAULT_BACKUP_PATH + "/"))
                    if (os.path.isfile(backup_file)):
                        self._load_launchers(self.get_xml_source(backup_file))
                elif (category == APPEND_COMMAND):
                    self._print_log(__language__(30185))
                    append_file = xbmcgui.Dialog().browse(1, __language__(30191), "files", ".xml", False, False,
                                                          os.path.join(PLUGIN_DATA_PATH + "/"))
    ...
    def _remove_category(self, categoryID):
        ...
        if (len(launcher_list) > 0):
            ret = dialog.yesno(__language__(30000),
                               __language__(30345) % (self.categories[categoryID]["name"], len(launcher_list)),
                               __language__(30346) % self.categories[categoryID]["name"],
                               __language__(30010) % self.categories[categoryID]["name"])
            if (ret):
    ...
    def _edit_rom(self, launcher, rom):
        ...
        type = dialog.select(__language__(30300) % title,
                             [__language__(30338), __language__(30301), __language__(30302), __language__(30303),
                              finished_display, __language__(30323), __language__(30304)])
        ...
        if (type == 1):
            ...
            type2 = dialog.select(__language__(30305),
                                  [__language__(30311) % self.settings["datas_scraper"], __language__(30333),
                                   __language__(30306) % self.launchers[launcher]["roms"][rom]["name"],
                                   __language__(30308) % self.launchers[launcher]["roms"][rom]["release"],
                                   __language__(30309) % self.launchers[launcher]["roms"][rom]["studio"],
                                   __language__(30310) % self.launchers[launcher]["roms"][rom]["genre"],
                                   __language__(30328) % self.launchers[launcher]["roms"][rom]["plot"][0:20],
                                   __language__(30316)])
            # Scrap rom Infos
            if (type2 == 0):
                self._scrap_rom(launcher, rom)
        ...
        if (type == 2):
            ...
            if (self.settings["thumbs_scraper"] == "GameFAQs") | (self.settings["thumbs_scraper"] == "MobyGames"):
                thumb_diag = __language__(30321) % (
                self.settings["thumbs_scraper"], self.settings["display_game_region"])
            if (self.settings["thumbs_scraper"] == "Google"):
                thumb_diag = __language__(30322) % (
                self.settings["thumbs_scraper"], self.settings["thumb_image_size_display"].capitalize())
            type2 = dialog.select(__language__(30302), [thumb_diag, __language__(30332), __language__(30313)])
            if (type2 == 0):
                self._scrap_thumb_rom(launcher, rom)
            if (type2 == 1):
                # Import a rom thumbnail image
                image = xbmcgui.Dialog().browse(2, __language__(30041), "files", ".jpg|.jpeg|.gif|.png", True, False,
                                                os.path.join(self.launchers[launcher]["thumbpath"]))
                if (image):
                        ...
                        if (img_ext != ''):
                            ...
                            if (self.launchers[launcher]["thumbpath"] == self.launchers[launcher]["fanartpath"]):
                                if (self.launchers[launcher]["thumbpath"] == self.launchers[launcher]["rompath"]):
                                    ...
                                    file_path = os.path.join(os.path.dirname(self.launchers[launcher]["thumbpath"]),
                                                             os.path.basename(
                            ...
                            if (image != file_path):
                                try:
                                    shutil.copy2(image.decode(get_encoding(), 'ignore'),
                                                 file_path.decode(get_encoding(), 'ignore'))
                                    ...
                                    xbmc_notify(__language__(30000),
                                                __language__(30063) % self.launchers[launcher]["roms"][rom]["name"],
                                                3000)
        ...
        if (type == 3):
            ...
            if (self.settings["fanarts_scraper"] == "Google"):
                fanart_diag = __language__(30322) % (
                self.settings["fanarts_scraper"], self.settings["fanart_image_size_display"].capitalize())
            type2 = dialog.select(__language__(30303), [fanart_diag, __language__(30332), __language__(30313)])
            if (type2 == 0):
                self._scrap_fanart_rom(launcher, rom)
            if (type2 == 1):
                # Import a rom fanart image
                image = xbmcgui.Dialog().browse(2, __language__(30042), "files", ".jpg|.jpeg|.gif|.png", True, False,
                                                os.path.join(self.launchers[launcher]["fanartpath"]))
                if (image):
                    if (os.path.isfile(image)):
                        img_ext = os.path.splitext(image)[-1][0:4]
                        if (img_ext != ''):
                            ...
                            if (self.launchers[launcher]["thumbpath"] == self.launchers[launcher]["fanartpath"]):
                                if (self.launchers[launcher]["fanartpath"] == self.launchers[launcher]["rompath"]):
                                    ...
                                    file_path = os.path.join(os.path.dirname(self.launchers[launcher]["fanartpath"]),
                                                             os.path.basename(
                                                                 filename.replace("." + filename.split(".")[-1],
                                                                                  '_fanart' + img_ext)))
                            else:
                            ...
                            if (image != file_path):
                                try:
                                    ...
                                    xbmc_notify(__language__(30000),
                                                __language__(30064) % self.launchers[launcher]["roms"][rom]["name"],
                                                3000)
            if (type2 == 2):
        ...
        if (type == 5):
            ...
            type2 = dialog.select(__language__(30323),
                                  [__language__(30337) % self.launchers[launcher]["roms"][rom]["filename"],
                                   __language__(30347) % self.launchers[launcher]["roms"][rom]["altapp"],
                                   __language__(30348) % self.launchers[launcher]["roms"][rom]["altarg"],
                                   __language__(30341) % self.launchers[launcher]["roms"][rom]["trailer"],
                                   __language__(30331) % self.launchers[launcher]["roms"][rom]["custom"]])
            if (type2 == 0):
                # Selection of the item file
                item_file = xbmcgui.Dialog().browse(1, __language__(30017), "files",
                                                    "." + self.launchers[launcher]["romext"].replace("|", "|."), False,
                                                    False, self.launchers[launcher]["roms"][rom]["filename"])
            ...
            if (type2 == 1):
                ...
                if (os.environ.get("OS", "xbox") == "xbox"):
                    ...
                    if (sys.platform == "win32"):
                        ...
                        filter = ""
                app = xbmcgui.Dialog().browse(1, __language__(30023), "files", filter, False, False,
                                              self.launchers[launcher]["roms"][rom]["altapp"])
            ...
            if (type2 == 3):
                ...
                trailer = xbmcgui.Dialog().browse(1, __language__(30090), "files", ".mp4|.mpg|.avi|.wmv|.mkv|.flv",
                                                  False, False, self.launchers[launcher]["roms"][rom]["trailer"])
                self.launchers[launcher]["roms"][rom]["trailer"] = trailer
                self._save_launchers()
            if (type2 == 4):
                # Selection of the rom customs path
                custom = xbmcgui.Dialog().browse(0, __language__(30057), "files", "", False, False,
                                                 self.launchers[launcher]["roms"][rom]["custom"])
    ...
    def _scrap_thumb_rom_algo(self, launcher, rom, title):
        xbmc_notify(__language__(30000), __language__(30065) % (
        self.launchers[launcher]["roms"][rom]["name"], (self.settings["thumbs_scraper"]).encode('utf-8', 'ignore')),
                    300000)
        xbmc.executebuiltin("ActivateWindow(busydialog)")
        covers = self._get_thumbnails_list(self.launchers[launcher]["roms"][rom]["gamesys"], title,
                                           self.settings["game_region"], self.settings["thumb_image_size"])
        if covers:
            nb_images = len(covers)
            xbmc.executebuiltin("Dialog.Close(busydialog)")
            xbmc_notify(__language__(30000),
                        __language__(30066) % (nb_images, self.launchers[launcher]["roms"][rom]["name"]), 3000)
            covers.insert(0, (
            self.launchers[launcher]["roms"][rom]["thumb"], self.launchers[launcher]["roms"][rom]["thumb"],
            __language__(30068)))
                    ...
                    if (img_url != ''):
                        ...
                        if (img_ext != ''):
                            ...
                            try:
                                ...
                                xbmc_notify(__language__(30000),
                                            __language__(30063) % self.launchers[launcher]["roms"][rom]["name"], 3000)
                    else:
                        xbmc_notify(__language__(30000),
                                    __language__(30067) % (self.launchers[launcher]["roms"][rom]["name"]), 3000)
        else:
            xbmc.executebuiltin("Dialog.Close(busydialog)")
            xbmc_notify(__language__(30000), __language__(30067) % (self.launchers[launcher]["roms"][rom]["name"]),
                        3000)

    def _scrap_thumb_rom(self, launcher, rom):
        if (self.launchers[launcher]["application"].lower().find('mame') > 0) or (
            self.settings["thumbs_scraper"] == 'arcadeHITS'):
    ...
    def _scrap_thumb_launcher_algo(self, launcherID, title):
        xbmc_notify(__language__(30000), __language__(30065) % (
        self.launchers[launcherID]["name"], (self.settings["thumbs_scraper"]).encode('utf-8', 'ignore')), 30000)
        covers = self._get_thumbnails_list(self.launchers[launcherID]["gamesys"], title, self.settings["game_region"],
                                           self.settings["thumb_image_size"])
        if covers:
            nb_images = len(covers)
            xbmc_notify(__language__(30000), __language__(30066) % (nb_images, self.launchers[launcherID]["name"]),
                        3000)
            covers.insert(0, (
            self.launchers[launcherID]["thumb"], self.launchers[launcherID]["thumb"], __language__(30068)))
            self.image_url = MyDialog(covers)
            if (self.image_url):
                    ...
                    if (img_url != ''):
                        ...
                        if (img_ext != ''):
                            ...
                            if (os.path.join(self.launchers[launcherID]["thumbpath"]) != ""):
                                file_path = os.path.join(self.launchers[launcherID]["thumbpath"], os.path.basename(
                                    self.launchers[launcherID]["application"]) + '_thumb' + img_ext)
                            else:
                                if (self.settings["launcher_thumb_path"] == ""):
                                    self.settings["launcher_thumb_path"] = DEFAULT_THUMB_PATH
                                file_path = os.path.join(self.settings["launcher_thumb_path"], os.path.basename(
                                    self.launchers[launcherID]["application"]) + '_thumb' + img_ext)
                            xbmc_notify(__language__(30000), __language__(30069), 300000)
                            try:
                                download_img(img_url, file_path)
                                ...
                                xbmc_notify(__language__(30000),
                                            __language__(30063) % self.launchers[launcherID]["name"], 3000)
                    else:
                        xbmc_notify(__language__(30000), __language__(30067) % (self.launchers[launcherID]["name"]),
                                    3000)
        else:
    ...
    def _scrap_thumb_category_algo(self, categoryID, title):
        xbmc_notify(__language__(30000), __language__(30065) % (
        self.categories[categoryID]["name"], (self.settings["thumbs_scraper"]).encode('utf-8', 'ignore')), 300000)
        covers = self._get_thumbnails_list("", title, "", self.settings["thumb_image_size"])
        if covers:
            nb_images = len(covers)
            xbmc_notify(__language__(30000), __language__(30066) % (nb_images, self.categories[categoryID]["name"]),
                        3000)
            covers.insert(0, (
            self.categories[categoryID]["thumb"], self.categories[categoryID]["thumb"], __language__(30068)))
            self.image_url = MyDialog(covers)
            if (self.image_url):
                if (not self.image_url == self.categories[categoryID]["thumb"]):
                    ...
                    if (img_url != ''):
                        ...
                        if (img_ext != ''):
                            ...
                            file_path = os.path.join(DEFAULT_THUMB_PATH, os.path.basename(
                                self.categories[categoryID]["name"]) + '_thumb' + img_ext)
                            xbmc_notify(__language__(30000), __language__(30069), 300000)
                            try:
                                download_img(img_url, file_path)
                                ...
                                xbmc_notify(__language__(30000),
                                            __language__(30063) % self.categories[categoryID]["name"], 3000)
                    else:
                        xbmc_notify(__language__(30000), __language__(30067) % (self.categories[categoryID]["name"]),
                                    3000)
        else:
    ...
    def _scrap_fanart_rom_algo(self, launcher, rom, title):
        xbmc_notify(__language__(30000), __language__(30071) % (
        self.launchers[launcher]["roms"][rom]["name"], self.settings["fanarts_scraper"].encode('utf-8', 'ignore')),
                    300000)
        full_fanarts = self._get_fanarts_list(self.launchers[launcher]["roms"][rom]["gamesys"], title,
                                              self.settings["fanart_image_size"])
        if full_fanarts:
            nb_images = len(full_fanarts)
            xbmc_notify(__language__(30000),
                        __language__(30072) % (nb_images, self.launchers[launcher]["roms"][rom]["name"]), 3000)
            full_fanarts.insert(0, (
            self.launchers[launcher]["roms"][rom]["fanart"], self.launchers[launcher]["roms"][rom]["fanart"],
            __language__(30068)))
                    ...
                    if (img_url != ''):
                        ...
                        if (img_ext != ''):
                            ...
                            try:
                                ...
                                xbmc_notify(__language__(30000),
                                            __language__(30064) % self.launchers[launcher]["roms"][rom]["name"], 3000)
                    else:
                        xbmc_notify(__language__(30000),
                                    __language__(30073) % (self.launchers[launcher]["roms"][rom]["name"]), 3000)
        else:
            xbmc_notify(__language__(30000), __language__(30073) % (self.launchers[launcher]["roms"][rom]["name"]),
                        3000)

    def _scrap_fanart_category_algo(self, categoryID, title):
        xbmc_notify(__language__(30000), __language__(30071) % (
        self.categories[categoryID]["name"], (self.settings["fanarts_scraper"]).encode('utf-8', 'ignore')), 300000)
        covers = self._get_fanarts_list("", title, self.settings["fanart_image_size"])
        if covers:
            nb_images = len(covers)
            xbmc_notify(__language__(30000), __language__(30072) % (nb_images, self.categories[categoryID]["name"]),
                        3000)
            covers.insert(0, (
            self.categories[categoryID]["fanart"], self.categories[categoryID]["fanart"], __language__(30068)))
            self.image_url = MyDialog(covers)
            if (self.image_url):
                if (not self.image_url == self.categories[categoryID]["fanart"]):
                    ...
                    if (img_url != ''):
                        ...
                        if (img_ext != ''):
                            filename = self.categories[categoryID]["name"]
                            file_path = os.path.join(DEFAULT_FANART_PATH, os.path.basename(
                                self.categories[categoryID]["name"]) + '_fanart' + img_ext)
                            xbmc_notify(__language__(30000), __language__(30074), 300000)
                            try:
                                download_img(img_url, file_path)
                                ...
                                xbmc_notify(__language__(30000),
                                            __language__(30064) % self.categories[categoryID]["name"], 3000)
                    else:
                        xbmc_notify(__language__(30000), __language__(30073) % (self.categories[categoryID]["name"]),
                                    3000)
        else:
    ...
    def _scrap_fanart_rom(self, launcher, rom):
        if (self.launchers[launcher]["application"].lower().find('mame') > 0) or (
            self.settings["fanarts_scraper"] == 'arcadeHITS'):
            title = os.path.basename(self.launchers[launcher]["roms"][rom]["filename"]).split(".")[0]
            keyboard = xbmc.Keyboard(title, __language__(30079))
        else:
            keyboard = xbmc.Keyboard(self.launchers[launcher]["roms"][rom]["name"], __language__(30036))
    ...
    def _scrap_fanart_launcher_algo(self, launcherID, title):
        xbmc_notify(__language__(30000), __language__(30071) % (
        self.launchers[launcherID]["name"], (self.settings["fanarts_scraper"]).encode('utf-8', 'ignore')), 300000)
        xbmc.executebuiltin("ActivateWindow(busydialog)")
        covers = self._get_fanarts_list(self.launchers[launcherID]["gamesys"], title,
                                        self.settings["fanart_image_size"])
        if covers:
            nb_images = len(covers)
            xbmc.executebuiltin("Dialog.Close(busydialog)")
            xbmc_notify(__language__(30000), __language__(30072) % (nb_images, self.launchers[launcherID]["name"]),
                        3000)
            covers.insert(0, (
            self.launchers[launcherID]["fanart"], self.launchers[launcherID]["fanart"], __language__(30068)))
            self.image_url = MyDialog(covers)
            if (self.image_url):
                    ...
                    if (img_url != ''):
                        ...
                        if (img_ext != ''):
                            ...
                            if (os.path.join(self.launchers[launcherID]["fanartpath"]) != ""):
                                file_path = os.path.join(self.launchers[launcherID]["fanartpath"], os.path.basename(
                                    self.launchers[launcherID]["application"]) + '_fanart' + img_ext)
                            else:
                                if (self.settings["launcher_fanart_path"] == ""):
                                    self.settings["launcher_fanart_path"] = DEFAULT_FANART_PATH
                                file_path = os.path.join(self.settings["launcher_fanart_path"], os.path.basename(
                                    self.launchers[launcherID]["application"]) + '_fanart' + img_ext)
                            xbmc_notify(__language__(30000), __language__(30074), 300000)
                            try:
                                download_img(img_url, file_path)
                                ...
                                xbmc_notify(__language__(30000),
                                            __language__(30064) % self.launchers[launcherID]["name"], 3000)
                    else:
                        xbmc_notify(__language__(30000), __language__(30073) % (self.launchers[launcherID]["name"]),
                                    3000)
        else:
            xbmc.executebuiltin("Dialog.Close(busydialog)")
    ...
    def _export_rom_nfo(self, launcher, rom):
        nfo_file = os.path.splitext(self.launchers[launcher]["roms"][rom]["filename"].decode(get_encoding()))[
                       0] + ".nfo"
        if (os.path.isfile(nfo_file)):
            ...
            for line in source:
                ...
                if len(item_title) > 0: line = "\t<title>" + self.launchers[launcher]["roms"][rom][
                    "name"] + "</title>\n"
                if len(item_platform) > 0: line = "\t<platform>" + self.launchers[launcher]["roms"][rom][
                    "gamesys"] + "</platform>\n"
                if len(item_year) > 0: line = "\t<year>" + self.launchers[launcher]["roms"][rom][
                    "release"] + "</year>\n"
                if len(item_publisher) > 0: line = "\t<publisher>" + self.launchers[launcher]["roms"][rom][
                    "studio"] + "</publisher>\n"
                if len(item_genre) > 0:
                    if first_genre == 0:
            ...
            xbmc_notify(__language__(30000), __language__(30087) % os.path.basename(nfo_file).encode('utf8', 'ignore'),
                        3000)
        else:
            nfo_content = "<game>\n\t<title>" + self.launchers[launcher]["roms"][rom][
                "name"] + "</title>\n\t<platform>" + self.launchers[launcher]["roms"][rom][
                              "gamesys"] + "</platform>\n\t<year>" + self.launchers[launcher]["roms"][rom][
                              "release"] + "</year>\n\t<publisher>" + self.launchers[launcher]["roms"][rom][
                              "studio"] + "</publisher>\n\t<genre>" + self.launchers[launcher]["roms"][rom][
                              "genre"] + "</genre>\n\t<plot>" + self.launchers[launcher]["roms"][rom][
                              "plot"] + "</plot>\n</game>\n"
            usock = open(nfo_file, 'w')
            usock.write(nfo_content)
            usock.close()
            xbmc_notify(__language__(30000), __language__(30086) % os.path.basename(nfo_file).encode('utf8', 'ignore'),
                        3000)
    ...
    def _modify_category(self, categoryID):
        dialog = xbmcgui.Dialog()
        type2 = dialog.select(__language__(30344), [__language__(30306) % self.categories[categoryID]["name"],
                                                    __language__(30310) % self.categories[categoryID]["genre"],
                                                    __language__(30328) % self.categories[categoryID]["plot"]])
        if (type2 == 0):
    ...
    def _edit_category(self, categoryID):
        ...
        if (self.categories[categoryID]["finished"] != "true"):
            ...
            finished_display = __language__(30340)
        type = dialog.select(__language__(30300) % self.categories[categoryID]["name"],
                             [__language__(30301), __language__(30302), __language__(30303), finished_display,
                              __language__(30304)])
        if (type == 0):
    ...
    def _edit_launcher(self, launcherID):
        ...
        if (self.launchers[launcherID]["rompath"] == ""):
            type = dialog.select(__language__(30300) % title,
                                 [__language__(30338), __language__(30301), __language__(30302), __language__(30303),
                                  __language__(30342) % self.categories[self.launchers[launcherID]["category"]]['name'],
                                  finished_display, __language__(30323), __language__(30304)])
        else:
            type = dialog.select(__language__(30300) % title,
                                 [__language__(30338), __language__(30301), __language__(30302), __language__(30303),
                                  __language__(30342) % self.categories[self.launchers[launcherID]["category"]]['name'],
                                  finished_display, __language__(30334), __language__(30323), __language__(30304)])
        type_nb = 0
        ...
        if (type == type_nb):
            dialog = xbmcgui.Dialog()
            type2 = dialog.select(__language__(30319), [__language__(30311) % self.settings["datas_scraper"],
                                                        __language__(30306) % self.launchers[launcherID]["name"],
                                                        __language__(30307) % self.launchers[launcherID]["gamesys"],
                                                        __language__(30308) % self.launchers[launcherID]["release"],
                                                        __language__(30309) % self.launchers[launcherID]["studio"],
                                                        __language__(30310) % self.launchers[launcherID]["genre"],
                                                        __language__(30328) % self.launchers[launcherID]["plot"][0:20],
                                                        __language__(30333), __language__(30316)])
            if (type2 == 0):
            ...
            if (type2 == 6):
                ...
                text_file = xbmcgui.Dialog().browse(1, __language__(30080), "files", ".txt|.dat", False, False,
                                                    self.launchers[launcherID]["application"])
                if (os.path.isfile(text_file) == True):
                    text_plot = open(text_file, 'r')
        ...
        if (type == type_nb):
            ...
            if (self.settings["thumbs_scraper"] == "Google"):
                thumb_diag = __language__(30322) % (
                self.settings["thumbs_scraper"], self.settings["thumb_image_size_display"])
            type2 = dialog.select(__language__(30302), [thumb_diag, __language__(30332), __language__(30313)])
            if (type2 == 0):
        ...
        if (self.launchers[launcherID]["rompath"] != ""):
            ...
            if (type == type_nb):
                dialog = xbmcgui.Dialog()
                type2 = dialog.select(__language__(30334),
                                      [__language__(30335), __language__(30336), __language__(30318), ])
        ...
        if (type == type_nb):
            ...
            if (self.launchers[launcherID]["rompath"] != ""):
                if (sys.platform == 'win32'):
                    type2 = dialog.select(__language__(30323),
                                          [__language__(30327) % self.launchers[launcherID]["application"],
                                           __language__(30315) % self.launchers[launcherID]["args"],
                                           __language__(30324) % self.launchers[launcherID]["rompath"],
                                           __language__(30317) % self.launchers[launcherID]["romext"],
                                           __language__(30325) % self.launchers[launcherID]["thumbpath"],
                                           __language__(30326) % self.launchers[launcherID]["fanartpath"],
                                           __language__(30341) % self.launchers[launcherID]["trailerpath"],
                                           __language__(30331) % self.launchers[launcherID]["custompath"],
                                           __language__(30329) % minimize_str, __language__(30330) % lnk_str])
                else:
                    type2 = dialog.select(__language__(30323),
                                          [__language__(30327) % self.launchers[launcherID]["application"],
                                           __language__(30315) % self.launchers[launcherID]["args"],
                                           ...
                                           __language__(30341) % self.launchers[launcherID]["trailerpath"],
                                           __language__(30331) % self.launchers[launcherID]["custompath"],
                                           __language__(30329) % minimize_str])
            else:
                if (sys.platform == 'win32'):
                    type2 = dialog.select(__language__(30323),
                                          [__language__(30327) % self.launchers[launcherID]["application"],
            ...
            type2_nb = 0
            if (type2 == type2_nb):
                app = xbmcgui.Dialog().browse(1, __language__(30023), "files", "", False, False,
                                              self.launchers[launcherID]["application"])
            ...
            if (self.launchers[launcherID]["rompath"] != ""):
                ...
                type2_nb = type2_nb + 1
                if (type2 == type2_nb):
                    rom_path = xbmcgui.Dialog().browse(0, __language__(30058), "files", "", False, False,
                                                       self.launchers[launcherID]["rompath"])
            ...
            if (type2 == type2_nb):
                thumb_path = xbmcgui.Dialog().browse(0, __language__(30059), "files", "", False, False,
                                                     self.launchers[launcherID]["thumbpath"])
            ...
            if (type2 == type2_nb):
                fanart_path = xbmcgui.Dialog().browse(0, __language__(30060), "files", "", False, False,
                                                      self.launchers[launcherID]["fanartpath"])
                self.launchers[launcherID]["fanartpath"] = fanart_path
            # Launcher trailer file menu option
            type2_nb = type2_nb + 1
            if (type2 == type2_nb):
                fanart_path = xbmcgui.Dialog().browse(1, __language__(30090), "files", ".mp4|.mpg|.avi|.wmv|.mkv|.flv",
                                                      False, False, self.launchers[launcherID]["trailerpath"])
                self.launchers[launcherID]["trailerpath"] = fanart_path
            # Launcher custom path menu option
            type2_nb = type2_nb + 1
            if (type2 == type2_nb):
                fanart_path = xbmcgui.Dialog().browse(0, __language__(30057), "files", "", False, False,
            ...
            if (type2 == type2_nb):
                dialog = xbmcgui.Dialog()
                type3 = dialog.select(__language__(30203), ["%s (%s)" % (__language__(30205), __language__(30201)),
                                                            "%s" % (__language__(30204))])
                if (type3 == 1):
            ...
            if (sys.platform == 'win32'):
                ...
                if (type2 == type2_nb):
                    dialog = xbmcgui.Dialog()
                    type3 = dialog.select(__language__(30206), ["%s (%s)" % (__language__(30204), __language__(30201)),
                                                                "%s (%s)" % (__language__(30205), __language__(30202))])
                    if (type3 == 1):
    ...
    def _import_launcher_nfo(self, launcherID):
        if (len(self.launchers[launcherID]["rompath"]) > 0):
            nfo_file = os.path.join(self.launchers[launcherID]["rompath"], os.path.basename(
                os.path.splitext(self.launchers[launcherID]["application"])[0] + ".nfo"))
        else:
            if (len(self.settings["launcher_nfo_path"]) > 0):
                nfo_file = os.path.join(self.settings["launcher_nfo_path"], os.path.basename(
                    os.path.splitext(self.launchers[launcherID]["application"])[0] + ".nfo"))
            else:
    ...
    def _export_launcher_nfo(self, launcherID):
        if (len(self.launchers[launcherID]["rompath"]) > 0):
            ...
            if (len(self.settings["launcher_nfo_path"]) > 0):
                ...
                nfo_file = os.path.join(nfo_path, os.path.basename(
                    os.path.splitext(self.launchers[launcherID]["application"])[0] + ".nfo"))
        if (os.path.isfile(nfo_file)):
            shutil.move(nfo_file, nfo_file + ".tmp")
            destination = open(nfo_file, "w")
            ...
            for line in source:
                ...
                if len(item_platform) > 0: line = "\t<platform>" + self.launchers[launcherID][
                    "gamesys"] + "</platform>\n"
                if len(item_year) > 0: line = "\t<year>" + self.launchers[launcherID]["release"] + "</year>\n"
                if len(item_publisher) > 0: line = "\t<publisher>" + self.launchers[launcherID][
                    "studio"] + "</publisher>\n"
            ...
            nfo_content = "<launcher>\n\t<title>" + self.launchers[launcherID]["name"] + "</title>\n\t<platform>" + \
                          self.launchers[launcherID]["gamesys"] + "</platform>\n\t<year>" + self.launchers[launcherID][
                              "release"] + "</year>\n\t<publisher>" + self.launchers[launcherID][
                              "studio"] + "</publisher>\n\t<genre>" + self.launchers[launcherID][
                              "genre"] + "</genre>\n\t<plot>" + self.launchers[launcherID][
                              "plot"] + "</plot>\n</launcher>\n"
            usock = open(nfo_file, 'w')
            usock.write(nfo_content)
            usock.close()
        ...
        if (self.launchers.has_key(launcherID)):
            ...
            if (os.path.basename(launcher["application"]).lower().replace(".exe", "") == "xbmc") or (
                ...
                if (os.path.exists(apppath)):
                        ...
                        if (sys.platform == 'win32'):
                            if (launcher["application"].split(".")[-1] == "lnk"):
                                ...
                                if (launcher["application"].split(".")[-1] == "bat"):
                                    ...
                                    info = None
                                startproc = subprocess_hack.Popen(r'%s %s' % (launcher["application"], arguments),
                    ...
                    xbmc_notify(__language__(30000) + " - " + __language__(30612),
    ...
    def _get_settings(self):
        ...
        self.settings["display_game_region"] = \
        [__language__(30136), __language__(30144), __language__(30145), __language__(30146)][
            int(__settings__.getSetting('game_region'))]
        self.settings["thumb_image_size"] = ['', 'icon', 'small', 'medium', 'large', 'xlarge', 'xxlarge', 'huge'][
            int(__settings__.getSetting('thumb_image_size'))]
        self.settings["thumb_image_size_display"] = \
        [__language__(30136), __language__(30137), __language__(30138), __language__(30139), __language__(30140),
         __language__(30141), __language__(30142), __language__(30143)][
            int(__settings__.getSetting('thumb_image_size'))]
        self.settings["fanart_image_size"] = ['', 'icon', 'small', 'medium', 'large', 'xlarge', 'xxlarge', 'huge'][
            int(__settings__.getSetting('fanart_image_size'))]
        ...
        self.settings["save_launchers"] = (__settings__.getSetting("save_launchers") == "true")
    ...
    def _get_scrapers(self):
        ...
        exec "import resources.scrapers.fanarts.%s.fanarts_scraper as _fanarts_scraper" % (
        self.settings["fanarts_scraper"])
            ...
            if (launcher["roms"].has_key(romName)):
                ...
                if (os.path.exists(apppath)):
                    if (os.path.exists(rompath)):
                        ...
                        for ext3 in ext3s:
                            ...
                            if (cleanromname.lower().find(ext3) > -1):
                                ...
                                try:
                                    ...
                                    xbmc_notify(__language__(30000) + " - " + __language__(30612), __language__(30610),
                                                3000)
                                ...
                                for filesname in filesnames:
                                    ...
                                    if searchname[0:len(namestem)] == namestem and searchname[
                                                                                   len(namestem):len(namestem) + len(
                                                                                           ext3) - 1] == ext3[:-1]:
                                        for romext in launcher["romext"].split("|"):
                                            if searchname[-len(romext):].lower() == romext.lower():
                                                Discnum = searchname[
                                                          (len(namestem) + len(ext3) - 1):searchname.rfind(".")]
                                                try:
                                                    int(Discnum)
                                                    if not altname:
                                                        files.append(
                                                            [Discnum, xbmc.getLocalizedString(427) + " " + Discnum,
                                                             os.path.join(os.path.dirname(rom["filename"]), filesname)])
                                                    else:
                                                        files.append([Discnum, altname[0][1:-1],
                                                                      os.path.join(os.path.dirname(rom["filename"]),
                                                                                   filesname)])
                                                except:
                                                    pass
                                if len(files) > 0:
                        ...
                        arguments = arguments.replace("%rombasename%", base_filename(romname)).replace("%ROMBASENAME%",
                                                                                                       base_filename(
                                                                                                           romname))
                        ...
                        arguments = arguments.replace("%romspath%", launcher["rompath"]).replace("%ROMSPATH%",
                                                                                                 launcher["rompath"])
                                ...
                                if (sys.platform == 'win32'):
                                    if (launcher["lnk"] == "true") and (launcher["romext"] == "lnk"):
                                        ...
                                        if (application.split(".")[-1] == "bat"):
                                            ...
                                            info = None
                                        startproc = subprocess_hack.Popen(r'%s %s' % (application, arguments),
                                                                          cwd=apppath, startupinfo=info)
                                        startproc.wait()
                                elif (sys.platform.startswith('linux')):
                                    if (self.settings["lirc_state"]):
                                    ...
                                    xbmc_notify(__language__(30000) + " - " + __language__(30612), __language__(30609),
                                                3000)
                            xbmc.sleep(self.settings["start_tempo"])
                            try:
                                xbmc.enableNavSounds(True)
                        ...
                        xbmc_notify(__language__(30000) + " - " + __language__(30612),
                                    __language__(30611) % os.path.basename(rom["filename"]), 3000)
                else:
                    xbmc_notify(__language__(30000) + " - " + __language__(30612),
                                __language__(30611) % os.path.basename(launcher["application"]), 3000)
    ...
    def _save_launchers(self):
        if not self.settings["save_launchers"]:
            return
        ...
        if (self.settings["auto_backup"]):
            ...
            if (os.path.isfile(BASE_CURRENT_SOURCE_PATH)):
                try:
                    ...
                    timestamp = str(now.year) + str(now.month).rjust(2, '0') + str(now.day).rjust(2, '0') + "-" + str(
                        now.hour).rjust(2, '0') + str(now.minute).rjust(2, '0') + str(now.second).rjust(2,
                                                                                                        '0') + "-" + str(
                        now.microsecond) + "-"
    ...
    def _load_launchers(self, xmlSource):
        ...
        if len(xml_categories) > 0:
            ...
            for category in categories:
                ...
                values = [re.findall("<id>(.*?)</id>", category), re.findall("<name>(.*?)</name>", category),
                          re.findall("<thumb>(.*?)</thumb>", category), re.findall("<fanart>(.*?)</fanart>", category),
                          re.findall("<genre>(.*?)</genre>", category),
                          re.findall("<description>(.*?)</description>", category),
                          re.findall("<finished>(.*?)</finished>", category)]
        ...
        # Else create the default category
            ...
            self.categories["default"] = {"id": "default", "name": "Default", "thumb": "", "fanart": "", "genre": "",
                                          "plot": "", "finished": "false"}
        ...
        if len(xml_launchers) > 0:
            ...
            for launcher in launchers:
                launcherdata = {}
                launcher_index = ["id", "name", "category", "application", "args", "rompath", "thumbpath", "fanartpath",
                                  "trailerpath", "custompath", "romext", "gamesys", "thumb", "fanart", "genre",
                                  "release", "studio", "plot", "finished", "minimize", "lnk", "roms"]
                values = [re.findall("<id>(.*?)</id>", launcher), re.findall("<name>(.*?)</name>", launcher),
                          re.findall("<category>(.*?)</category>", launcher),
                          re.findall("<application>(.*?)</application>", launcher),
                          re.findall("<args>(.*?)</args>", launcher), re.findall("<rompath>(.*?)</rompath>", launcher),
                          re.findall("<thumbpath>(.*?)</thumbpath>", launcher),
                          re.findall("<fanartpath>(.*?)</fanartpath>", launcher),
                          re.findall("<trailerpath>(.*?)</trailerpath>", launcher),
                          re.findall("<custompath>(.*?)</custompath>", launcher),
                          re.findall("<romext>(.*?)</romext>", launcher),
                          re.findall("<platform>(.*?)</platform>", launcher),
                          re.findall("<thumb>(.*?)</thumb>", launcher), re.findall("<fanart>(.*?)</fanart>", launcher),
                          re.findall("<genre>(.*?)</genre>", launcher),
                          re.findall("<release>(.*?)</release>", launcher),
                          re.findall("<publisher>(.*?)</publisher>", launcher),
                          re.findall("<launcherplot>(.*?)</launcherplot>", launcher),
                          re.findall("<finished>(.*?)</finished>", launcher),
                          re.findall("<minimize>(.*?)</minimize>", launcher), re.findall("<lnk>(.*?)</lnk>", launcher),
                          re.findall("<roms>(.*?)</roms>", launcher)]
                ...
                if len(roms) > 0:
                    for rom in roms:
                        romdata = {}
                        rom_index = ["id", "name", "filename", "thumb", "fanart", "trailer", "custom", "genre",
                                     "release", "studio", "plot", "finished", "altapp", "altarg"]
                        r_values = [re.findall("<id>(.*?)</id>", rom), re.findall("<name>(.*?)</name>", rom),
                                    re.findall("<filename>(.*?)</filename>", rom),
                                    re.findall("<thumb>(.*?)</thumb>", rom), re.findall("<fanart>(.*?)</fanart>", rom),
                                    re.findall("<trailer>(.*?)</trailer>", rom),
                                    re.findall("<custom>(.*?)</custom>", rom), re.findall("<genre>(.*?)</genre>", rom),
                                    re.findall("<release>(.*?)</release>", rom),
                                    re.findall("<publisher>(.*?)</publisher>", rom),
                                    re.findall("<gameplot>(.*?)</gameplot>", rom),
                                    re.findall("<finished>(.*?)</finished>", rom),
                                    re.findall("<altapp>(.*?)</altapp>", rom),
                                    re.findall("<altarg>(.*?)</altarg>", rom)]
        ...
        for key in sorted(self.categories, key=lambda x: self.categories[x]["name"]):
            if (not self.settings["hide_default_cat"] or self.categories[key]['id'] != "default"):
                self._add_category(self.categories[key]["name"], self.categories[key]["thumb"],
                                   ...
                                   self.categories[key]["plot"], self.categories[key]["finished"], len(self.categories),
                                   key)
        ...
        for key in sorted(self.launchers, key=lambda x: self.launchers[x]["application"]):
            if (self.launchers[key]["category"] == categoryID):
                self._add_launcher(self.launchers[key]["name"], self.launchers[key]["category"],
                                   ...
                                   self.launchers[key]["trailerpath"], self.launchers[key]["custompath"],
        ...
        if (self.launchers.has_key(launcherID)):
            ...
            if (len(roms) != 0):
                for key in sorted(roms, key=lambda x: roms[x]["filename"]):
                    ...
                    self._add_rom(launcherID, roms[key]["name"], roms[key]["filename"], roms[key]["gamesys"],
                                  roms[key]["thumb"], defined_fanart, roms[key]["trailer"], roms[key]["custom"],
                                  roms[key]["genre"], roms[key]["release"], roms[key]["studio"], roms[key]["plot"],
                                  roms[key]["finished"], roms[key]["altapp"], roms[key]["altarg"], len(roms), key,
                                  False, "")
                xbmcplugin.endOfDirectory(handle=int(self._handle), succeeded=True, cacheToDisc=False)
            else:
    ...
    def _import_roms(self, launcherID, addRoms=False):
        ...
        for fullname in files:
            ...
            if (self.settings["datas_method"] == "2"):
                import_text = __language__(30061) % (
                f.replace("." + f.split(".")[-1], ""), self.settings["datas_scraper"].encode('utf-8', 'ignore'))
            if (self.settings["datas_method"] == "3"):
                import_text = __language__(30084) % (
                f.replace("." + f.split(".")[-1], ""), self.settings["datas_scraper"].encode('utf-8', 'ignore'))
            ...
            for ext in exts.split("|"):
                ...
                if f.upper().endswith("." + ext.upper()):
                    ...
                    if (foundromfile == False):
                        ...
                        if (self.settings["datas_method"] == "2") or (
                            (self.settings["datas_method"] == "3") and (found_nfo == 0)):
                            ...
                            if (app.lower().find('mame') > 0) or (self.settings["datas_scraper"] == 'arcadeHITS'):
                                ...
                                if (self.settings["scrap_info"] == "1"):
                                    ...
                                    if display:
                                        ...
                                        dialog = xbmcgui.Dialog()
                                        # Game selection
                                        selectgame = dialog.select(
                                            __language__(30078) % (self.settings["datas_scraper"]), display)
                                        if (selectgame == -1):
                                            results = []
                            if results:
                        ...
                        if (self.settings["thumbs_method"] == "2"):
                            ...
                            if (self.settings["overwrite_thumbs"]) or (thumb == ""):
                                pDialog.update(filesCount * 100 / len(files), __language__(30065) % (
                                f.replace("." + f.split(".")[-1], ""),
                                ...
                                if (app.lower().find('mame') > 0) or (self.settings["thumbs_scraper"] == 'arcadeHITS'):
                                    covers = self._get_thumbnails_list(romdata["gamesys"], title,
                                                                       self.settings["game_region"],
                                                                       self.settings["thumb_image_size"])
                                else:
                                    covers = self._get_thumbnails_list(romdata["gamesys"], romdata["name"],
                                                                       self.settings["game_region"],
                                                                       self.settings["thumb_image_size"])
                                if covers:
                                    if (self.settings["scrap_thumbs"] == "1"):
                                        if (self.settings["snap_flyer"] == "1") and (
                                            self.settings["thumbs_scraper"] == 'arcadeHITS'):
                                        ...
                                        if (self.image_url):
                                            img_url = self._get_thumbnail(self.image_url)
                                            ret = pDialog.create(__language__(30000), __language__(30014) % (path))
                                            pDialog.update(filesCount * 100 / len(files), __language__(30061) % (
                                            f.replace("." + f.split(".")[-1], ""),
                                    ...
                                    if (img_url != ''):
                                        try:
                                            ...
                                            shutil.copy2(thumb.decode(get_encoding(), 'ignore'),
                                                         cached_thumb.decode(get_encoding(), 'ignore'))
                                        except socket.timeout:
                                            xbmc_notify(__language__(30000) + " - " + __language__(30612),
                                                        __language__(30604), 3000)
                                        except exceptions.IOError:
                                            xbmc_notify(__language__(30000) + " - " + __language__(30612),
                                                        __language__(30605), 3000)
                                    else:
                            ...
                            if (self.settings["thumbs_method"] == "0"):
                                ...
                                pDialog.update(filesCount * 100 / len(files), __language__(30065) % (
                                f.replace("." + f.split(".")[-1], ""), __language__(30172)))
                                romdata["thumb"] = thumb
                        ...
                        if (self.settings["fanarts_method"] == "2"):
                            ...
                            if (self.settings["overwrite_fanarts"]) or (fanart == ""):
                                pDialog.update(filesCount * 100 / len(files), __language__(30071) % (
                                f.replace("." + f.split(".")[-1], ""),
                                ...
                                if (app.lower().find('mame') > 0) or (self.settings["fanarts_scraper"] == 'arcadeHITS'):
                                    covers = self._get_fanarts_list(romdata["gamesys"], title,
                                                                    self.settings["fanart_image_size"])
                                else:
                                    covers = self._get_fanarts_list(romdata["gamesys"], romdata["name"],
                                                                    self.settings["fanart_image_size"])
                                if covers:
                                    ...
                                    if (img_url != ''):
                                        try:
                                            ...
                                            shutil.copy2(fanart.decode(get_encoding(), 'ignore'),
                                                         cached_thumb.decode(get_encoding(), 'ignore'))
                                        except socket.timeout:
                                            xbmc_notify(__language__(30000) + " - " + __language__(30612),
                                                        __language__(30606), 3000)
                                        except exceptions.IOError:
                                            xbmc_notify(__language__(30000) + " - " + __language__(30612),
                                                        __language__(30607), 3000)
                                    else:
                        ...
                        if (addRoms):
                            self._add_rom(launcherID, romdata["name"], romdata["filename"], romdata["gamesys"],
                                          romdata["thumb"], romdata["fanart"], romdata["trailer"], romdata["custom"],
                                          romdata["genre"], romdata["release"], romdata["studio"], romdata["plot"],
                                          romdata["finished"], romdata["altapp"], romdata["altarg"], len(files), key,
                                          False, "")
                            romadded = True
            if not romadded:
    ...
    def _add_category(self, name, thumb, fanart, genre, plot, finished, total, key):
        ...
        if (finished != "true") or (self.settings["hide_finished"] == False):
            xbmcplugin.addDirectoryItem(handle=int(self._handle), url="%s?%s" % (self._path, key), listitem=listitem,
                                        isFolder=True)

    def _add_launcher(self, name, category, cmd, path, thumbpath, fanartpath, trailerpath, custompath, ext, gamesys,
                      thumb, fanart, genre, release, studio, plot, finished, lnk, minimize, roms, total, key):
        ...
        commands.append(
            (__language__(30051), "XBMC.RunPlugin(%s?%s/%s)" % (self._path, category, FILE_MANAGER_COMMAND),))
        commands.append((__language__(30101), "XBMC.RunPlugin(%s?%s/%s)" % (self._path, category, ADD_COMMAND),))
        commands.append(
            (__language__(30109), "XBMC.RunPlugin(%s?%s/%s/%s)" % (self._path, category, key, EDIT_COMMAND),))
        ...
        if (path == ""):
            ...
            icon = "DefaultFolder.png"
            commands.append(
                (__language__(30106), "XBMC.RunPlugin(%s?%s/%s/%s)" % (self._path, category, key, ADD_COMMAND),))
        ...
        listitem.setInfo("video",
                         {"Title": name, "Label": os.path.basename(cmd), "Plot": plot, "Studio": studio, "Genre": genre,
                          "Premiered": release, display_date_format: release, "Writer": gamesys,
                          "Trailer": os.path.join(trailerpath), "Director": os.path.join(custompath),
    ...
    def _add_rom(self, launcherID, name, cmd, romgamesys, thumb, romfanart, romtrailer, romcustom, romgenre, romrelease,
                 romstudio, romplot, finished, altapp, altarg, total, key, search, search_url):
        if (int(xbmc.getInfoLabel("System.BuildVersion")[0:2]) < 12):
            # Dharma / Eden compatible
            display_date_format = "Date"
        else:
        ...
        listitem.setInfo("video", {"Title": name, "Label": os.path.basename(cmd), "Plot": romplot, "Studio": romstudio,
                                   "Genre": romgenre, "Premiered": romrelease, display_date_format: romrelease,
                                   "Writer": romgamesys, "Trailer": os.path.join(romtrailer),
                                   "Director": os.path.join(romcustom), "overlay": ICON_OVERLAY})

        commands = []
        commands.append((__language__(30512), "XBMC.RunPlugin(%s?%s/%s/%s)" % (
        self._path, self.launchers[launcherID]["category"], launcherID, SEARCH_COMMAND),))
        commands.append((__language__(30107), "XBMC.RunPlugin(%s?%s/%s/%s/%s)" % (
        self._path, self.launchers[launcherID]["category"], launcherID, key, EDIT_COMMAND),))
        if search:
            commands.append((__language__(30513), "XBMC.RunPlugin(%s?%s/%s/%s)" % (
            self._path, self.launchers[launcherID]["category"], launcherID, SEARCH_COMMAND),))
        listitem.addContextMenuItems(commands)
        if (finished != "true") or (self.settings["hide_finished"] == False):
            xbmcplugin.addDirectoryItem(handle=int(self._handle), url="%s?%s/%s/%s" % (
            self._path, self.launchers[launcherID]["category"], launcherID, key), listitem=listitem, isFolder=False)
    ...
    def _add_new_rom(self, launcherID):
        ...
        if (romfile):
            ...
            keyboard = xbmc.Keyboard(title.replace('.' + title.split('.')[-1], '').replace('.', ' '),
                                     __language__(30018))
            keyboard.doModal()
            if (keyboard.isConfirmed()):
                title = keyboard.getText()
                ...
                for ext2 in ext2s:
                    if (thumb_path == fanart_path):
                        if (thumb_path == rompath):
                            if (os.path.isfile(os.path.join(os.path.dirname(romfile),
                                                            f.replace("." + f.split(".")[-1], '_thumb.' + ext2)))):
                                romthumb = os.path.join(os.path.dirname(romfile),
                                                        f.replace("." + f.split(".")[-1], '_thumb.' + ext2))
                        else:
                            if (os.path.isfile(
                                    os.path.join(thumb_path, f.replace("." + f.split(".")[-1], '_thumb.' + ext2)))):
                    ...
                    if (fanart_path == thumb_path):
                        if (fanart_path == rompath):
                            if (os.path.isfile(os.path.join(os.path.dirname(romfile),
                                ...
                                romfanart = os.path.join(os.path.dirname(romfile),
                                                         f.replace("." + f.split(".")[-1], '_fanart.' + ext2))
                        else:
                            if (os.path.isfile(
                                    os.path.join(fanart_path, f.replace("." + f.split(".")[-1], '_fanart.' + ext2)))):
                                romfanart = os.path.join(fanart_path,
                                                         f.replace("." + f.split(".")[-1], '_fanart.' + ext2))
                    else:
                        if (fanart_path == ""):
                            romfanart = os.path.join(os.path.dirname(romfile),
    ...
    def _add_new_launcher(self, categoryID):
        if (self.categories.has_key(categoryID)):
            dialog = xbmcgui.Dialog()
            type = dialog.select(__language__(30101),
                                 [__language__(30021), __language__(30022), __language__(30026), __language__(30027),
                                  __language__(30051)])
            if (os.environ.get("OS", "xbox") == "xbox"):
                filter = ".xbe|.cut"
            else:
            ...
            if (type == 0):
                ...
                if (app):
                    ...
                    if (sys.platform == "win32"):
                        ...
                        launcher_lnk = ""
                    launcherdata = {"name": title, "category": categoryID, "application": app, "args": args,
                                    "rompath": "", "thumbpath": thumb_path, "fanartpath": fanart_path, "custompath": "",
                                    "trailerpath": "", "romext": "", "gamesys": launcher_gamesys, "thumb": "",
                                    "fanart": "", "genre": "", "release": "", "studio": "", "plot": "",
                                    "finished": "false", "lnk": launcher_lnk, "minimize": "false", "roms": {}}
                    ...
                    if (path):
                        ...
                        if (extkey.isConfirmed()):
                            ...
                            launcherdata = {"name": title, "category": categoryID, "application": app, "args": args,
                                            ...
                                            "custompath": "", "trailerpath": "", "romext": ext,
                                            "gamesys": launcher_gamesys, "thumb": "", "fanart": "", "genre": "",
                                            "release": "", "studio": "", "plot": "", "finished": "false",
                                            "lnk": launcher_lnk, "minimize": "false", "roms": {}}
                            # add launcher to the launchers list (using name as index)
                            launcherid = _get_SID()
                            self.launchers[launcherid] = launcherdata
                            self._save_launchers()
            ...
            if (type == 2):
                ...
                if (launcher_query):
                    ...
                    launcherdata = {"name": title, "category": categoryID, "application": app, "args": args,
                                    "rompath": "", "thumbpath": DEFAULT_THUMB_PATH, "fanartpath": DEFAULT_FANART_PATH,
                                    "custompath": "", "trailerpath": "", "romext": "", "gamesys": "xbmc", "thumb": "",
                                    "fanart": "", "genre": "", "release": "", "studio": "", "plot": "",
                                    "finished": "false", "lnk": "", "minimize": "false", "roms": {}}
                    # add launcher to the launchers list (using name as index)
                    launcherid = _get_SID()
                    self.launchers[launcherid] = launcherdata
                    self._save_launchers()
            ...
            if (type == 3):
                ...
                if (favourite_url != -1):
                    ...
                    launcherdata = {"name": title, "category": categoryID, "application": app, "args": args,
                                    "rompath": "", "thumbpath": DEFAULT_THUMB_PATH, "fanartpath": DEFAULT_FANART_PATH,
                                    "custompath": "", "trailerpath": "", "romext": "", "gamesys": "xbmc",
                                    "thumb": thumb, "fanart": "", "genre": "", "release": "", "studio": "", "plot": "",
                                    "finished": "false", "lnk": "", "minimize": "false", "roms": {}}
                    # add launcher to the launchers list (using name as index)
                    launcherid = _get_SID()
                    self.launchers[launcherid] = launcherdata
    ...
    def _find_roms(self, is_launcher):
        dialog = xbmcgui.Dialog()
        type = dialog.select(__language__(30400),
                             [__language__(30401), __language__(30402), __language__(30403), __language__(30404),
                              __language__(30405)])
        type_nb = 0
        ...
        if (type == type_nb):
            ...
            if (not selected == -1):
                if (is_launcher):
                    ...
                    xbmc.executebuiltin(
                        "ReplaceWindow(Programs,%s?%s/%s)" % (self._path, search[selected], SEARCH_DATE_COMMAND))
        ...
        if (type == type_nb):
            ...
            if (not selected == -1):
                if (is_launcher):
                    ...
                    xbmc.executebuiltin(
                        "ReplaceWindow(Programs,%s?%s/%s)" % (self._path, search[selected], SEARCH_PLATFORM_COMMAND))
        ...
        if (type == type_nb):
            ...
            if (not selected == -1):
                if (is_launcher):
                    ...
                    xbmc.executebuiltin(
                        "ReplaceWindow(Programs,%s?%s/%s)" % (self._path, search[selected], SEARCH_STUDIO_COMMAND))
        ...
        if (type == type_nb):
            ...
            if (not selected == -1):
                if (is_launcher):
                    ...
                    xbmc.executebuiltin(
                        "ReplaceWindow(Programs,%s?%s/%s)" % (self._path, search[selected], SEARCH_GENRE_COMMAND))
...
def _find_category_roms(self, search, category):
    ...
    for key in sorted(rl.iterkeys()):
        self._add_rom(rl[key]["launcherID"], rl[key]["name"], rl[key]["filename"], rl[key]["gamesys"], rl[key]["thumb"],
                      rl[key]["fanart"], rl[key]["trailer"], rl[key]["custom"], rl[key]["genre"], rl[key]["release"],
                      rl[key]["studio"], rl[key]["plot"], rl[key]["finished"], rl[key]["altapp"], rl[key]["altarg"],
                      len(rl), key, True, s_url)
    xbmcplugin.endOfDirectory(handle=int(self._handle), succeeded=True, cacheToDisc=False)
